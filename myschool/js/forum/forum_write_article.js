							...
							if(data == true) {
								...
								setInterval(function() {
									...
									if(time / 1000 == 0) {
										...
										mui.fire(pageTo,'listener_personal_id',{uesr_id:user.id})
										mui.openWindow({
...
$('#content').keyup(function(event) {
	...
	var _3  = false
	var _4  = false
	_index__1 = _index__1 != -1 ? _index__1 : texts.indexOf('# ')
	if(texts.indexOf('# ')!=-1){
		_1 = true
	}
	_index__1 = _index__1 != -1 ? _index__1 : texts.indexOf('## ')
	...
	if(_index__1 != -1) {
		...
		if(br_index!=-1){//如果出现
			...
			var  text_r =texts_sub_1.substring(2,br_index) //需要替换的字符串
...
document.getElementById('back_a').addEventListener('tap', function() {
	...
	document.getElementById('content').innerHTML = '<span id="span" class="content-font">主要内容</span>'
})