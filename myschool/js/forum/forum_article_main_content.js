 ...
 模拟用户登录
 * */
var service_url = "http://172.19.129.1:86/"
//当前用户
...
window.addEventListener('listener_aritcle_id', function(event) {
	...
	mui.ajax({
		...
		error: function(http, er) {
			if(http.status == 404) {
				mui.alert('找不到页面', ' ', 500, 'div')
			}
			if(http.timeout) {
				mui.alert('链接超时', ' ', 500, 'div')
			}
			if(er == 'error') {
				mui.alert('服务器爆炸1', ' ', 500, 'div')
			}
		}
	})
})
			...
			if(type == 1) {
					...
					success: function(data, su, http) {
						if(http.readyState == 4 && http.status == 200) {
							...
							if(data == true) {
								...
								var li = '<li  title="' + user.id + '" name="' + user.nickname + '" class="mui-table-view-cell mui-media">'
								...
								li += '<span class="difination-span-left">' + new Date().Format('yyyy-MM-dd hh:mm:ss') + '</span></p>'
								li += '<label style="font-size: 13px;">' + reply + '</label></div><a id="' + reply + '" href="javascript.void(0)"></a></li>'
								ul.innerHTML = li
								taple.appendChild(ul)
								//修改没有评论  
								document.querySelector('.difination-maggin-top40px').innerText = '评论'
							}
						}
				...
				mui.ajax({
					...
					success: function(data, su, http) {
						...
						if(http.readyState == 4 && http.status == 200) {
							if(data == true) {
								var reply_table = document.querySelector('.' + comment_ranks)
								//console.log(reply_table.innerHTML)
								...
								span += '<span class="difination-span-left">' + new Date().Format('yyyy-MM-dd hh:mm:ss') + '</span>'
								span += '</p></div></li>'
								li.innerHTML = span
...
mui('#main').on('tap', 'mind>button', function() {
	...
	if(type == 'yes') {
		...
		mui.ajax({
			...
			error: function(http, er) {
				if(http.status == 404) {
					mui.alert('找不到网页', ' ', 500, 'div')
				} else if(http.status == 500) {
					mui.alert('服务器爆炸3', ' ', 500, 'div')
				} else if(http.timeout) {
					mui.alert('网络不好', ' ', 500, 'div')
				}
			}
